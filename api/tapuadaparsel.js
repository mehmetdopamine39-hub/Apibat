...
//http://localhost:3000/api/devrelax/tapuadaparsel?il=ADANA&ilce=ALADAĞ&mahalle=AKPINAR&ada=186&parsel=1&auth=discord.gg/relaxservices
router.get("/tapuadaparsel", limiter, async (req, res) => {
    try {
        const { il, ilce, mahalle, ada, parsel, auth } = req.query;

        if (auth !== "discord.gg/relaxservices") {
            return res.status(401).json({ uyari: "Yetkisiz Erisim!" });
        }
        if (!il || !ilce || !mahalle || !ada || !parsel) {
            return res.status(400).json({ uyari: "Lutfen Tum Parametreleri, Eksiksiz Giriniz!" });
        }

        const query = `
            SELECT * FROM takbis
            WHERE İlBilgisi = ? AND İlceBilgisi = ? AND MahalleBilgisi LIKE ? 
            AND AdaBilgisi = ? AND ParselBilgisi = ?
        `;
        const params = [il, ilce, `%${mahalle}%`, ada, parsel];
